...
void setup() {
  ...
  delay(3000);
  steadyState(duration,tolerance);
}
...
//    Serial.print("333");
    for (byte s = 0; s < NUM_SENSORS; s++) {
      Serial.println(abs(mcp[s].readThermocouple() - initial[s]));
      if (abs(mcp[s].readThermocouple() - initial[s]) > tolerance) {
        return false;
      }
    }
    delay(100);
  }  
  Serial.print("Steady State Reached! \n");
  return true;
}
