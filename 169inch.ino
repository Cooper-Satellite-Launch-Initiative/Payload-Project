...
//#include"image.h"
// pin definitions
#define BUSY_PIN    2    // Busy信号
#define NRST_PIN    3    // nRST
#define NDC_PIN     4    // nDC
#define CSB_PIN     5    // CSB (Master CS)
#define SCLK_PIN    11   // SCLK 
#define MOSI_PIN    12  // MOSI (也用作MISO) //SDA
#define CSB2_PIN    8    // CSB2 (Slave CS)
#define MS_PIN      9 // MS


// Mode Definition (Revised to match function logic: 0 - Master IC Exclusive, 1 - Slave IC Exclusive, 2 - Shared by Master and Slave ICs)
...
#define PIC_HALF            0xFC        //half picture
#define PIC_A               0xFD        //full picture
#define STRIPE              0xFE
#define IMAGE               0xFF
...
void Sys_init(void) {
  ...
  // ensure the initial state of all pins is correct
...
void Reset(void) {
  ...
  Delay_ms(30);
  nRST_H;
  Delay_ms(100); // increase waiting time after resetting
...
void SPI4W_WriteCom(unsigned char init_com) {
  ...
  pinMode(MOSI_PIN, OUTPUT); // ensure it is set to output mode before writing
  temp_com = init_com;
  SCLK_L;
  delayMicroseconds(2);
  nDC_L;
  ...
  for (scnt = 0; scnt < 8; scnt++) {
    ...
    delayMicroseconds(1);
    SCLK_H;
    delayMicroseconds(2); // Increase SCLK high-level duration.
    SCLK_L;
    temp_com = temp_com << 1;
  }
  delayMicroseconds(2);
...
unsigned char SPI4W_ReadData(void) {
  ...
  pinMode(MOSI_PIN, INPUT); // set as input during reading
  delayMicroseconds(1);
  SCLK_L;
  delayMicroseconds(2);
  nDC_H;
  ...
  pinMode(MOSI_PIN, OUTPUT); // Restore to output after reading.
  return temp;
}
...
void MsDev_WriteData(unsigned char MS_opt, unsigned char init_data) {
  ...
  delayMicroseconds(10); // Increase the delay after CS switching.
